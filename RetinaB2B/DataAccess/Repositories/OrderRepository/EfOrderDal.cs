    ...
    public class EfOrderDal : EfEntityRepositoryBase<Order, SimpleContextDb>, IOrderDal
    {
            ...
            using (var context = new SimpleContextDb())
            {
                var result = from order in context.Orders.Where(p => p.Id == id)
                             ...
                             select new OrderDto
                             {
                                 ...
                                 Total = context.OrderDetails.Where(p => p.OrderId == order.Id).Sum(s => s.Price) *
                                 context.OrderDetails.Where(p => p.OrderId == order.Id).Sum(s => s.Quantity)
                             };
        ...
        public async Task<List<OrderDto>> GetListByCustomerIdDto(int customerId)
        {
            using (var context = new SimpleContextDb())
            {
                ...
                return await result.OrderByDescending(p => p.Quantity).ToListAsync();
            }
        }
