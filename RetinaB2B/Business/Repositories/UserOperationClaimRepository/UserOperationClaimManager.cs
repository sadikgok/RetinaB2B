    ...
    public class UserOperationClaimManager : IUserOperationClaimService
    {
        ...
        public async Task<IResult> Update(UserOperationClaim userOperationClaim)
        {
            IResult result = BusinessRules.Run(
                ...
                await IsOperationClaimExist(userOperationClaim.UserOperationClaimId),
        ...
        {
            var result = await _userOperationClaimDal.Get(p => p.UserId == userOperationClaim.UserId && p.UserOperationClaimId == userOperationClaim.UserOperationClaimId);
            if (result != null)
            {
        ...
        private async Task<IResult> IsOperationSetExistForUpdate(UserOperationClaim userOperationClaim)
        {
            ...
            if (currentUserOperationClaim.UserId != userOperationClaim.UserId || currentUserOperationClaim.UserOperationClaimId != userOperationClaim.UserOperationClaimId)
            {
                var result = await _userOperationClaimDal.Get(p => p.UserId == userOperationClaim.UserId && p.UserOperationClaimId == userOperationClaim.UserOperationClaimId);
                if (result != null)
                {
